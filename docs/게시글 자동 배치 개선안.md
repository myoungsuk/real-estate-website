...
flowchart TD
    ...
    SYNC --> YT["YouTube<br/>Data API v3"]
...
공개 저장소의 표준 GitHub-hosted runner는 현재 무료이며, 매시간 `channels.list` 1회와 `playlistItems.list` 1회만 호출하면 약 48 quota unit/day다. 각 list 호출은 1 unit이고 기본 일일 할당량 10,000 unit에 비해 작다. 페이지네이션이나 추가 `videos.list` 호출이 생기면 실제 사용량을 다시 계산한다.
...
1차 커밋: workflow_dispatch만 포함
  ↓
Secret·branch protection/ruleset 확인
  ↓
...
1차 코드에 비활성 schedule을 주석으로 남기거나, Secret이 없는데 schedule부터 넣지 않는다.
...
공식 API에서 다음 값을 확인할 수 있다.

- `videoId`
- 제목
- 설명
- YouTube 공개 시각인 `contentDetails.videoPublishedAt`
- 썸네일

`snippet.publishedAt`은 영상 공개 시각이 아니라 업로드 재생목록에 항목이 추가된 시각이므로 `publishedAt` 저장값으로 사용하지 않는다.
...
브라우저 코드나 저장소 JSON에 API Key를 넣지 않는다.
...
채널 ID는 코드에 공개 설정값으로 둘 수 있으나, 실제 값은 구현 전에 공식 채널과 일치하는지 다시 검증한다.

각 항목은 다음을 모두 만족해야 한다.

- `status.privacyStatus === "public"`
- `snippet.videoOwnerChannelId`가 승인된 channelId와 일치
- `contentDetails.videoId`와 watch URL의 `v` 값이 일치
- `contentDetails.videoPublishedAt`이 유효한 ISO 8601 시각
...
```json
{
  ...
  "status": "published",
  "title": "{YouTube 공식 제목}",
  "summary": "공식 YouTube 채널에서 “{제목}” 영상을 확인해 보세요.",
...
현재 40개 YouTube 항목은 원본 `videoId`를 소문자 kebab-case로 정규화한 ID와 일치한다. 그러나 `_`, `-`, 대문자 정규화 과정에서 서로 다른 원본 ID가 같은 내부 ID가 될 수 있으므로 다음 충돌 검사를 추가한다.
...
원본 `videoId`는 새 스키마 필드로 임의 추가하지 않고 공식 watch URL에서 추출해 비교한다. 스키마를 바꿀 필요가 확인되면 핵심 문서·검증기·관리자 편집 화면을 함께 갱신한다.
...
- 비공개 YouTube 영상 또는 승인 채널 불일치 → 추가 금지
...
현재 콘텐츠 규모와 API quota에서는 매시간 확인 부담이 작다. 45일 keepalive와 월 1회 상태 확인으로 장기 중단 위험을 관리한다.
...
- [ ] YouTube 공개 시각은 `contentDetails.videoPublishedAt`을 사용
- [ ] 승인 채널·공개 상태·원본 videoId 충돌 검사 통과
...
- [ ] 운영 문서에 수동 실행·실패 확인·Secret 설정 절차 기록
...
- 공식 YouTube channelId
- GitHub Actions Secret: YOUTUBE_API_KEY
