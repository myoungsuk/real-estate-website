...
- 한 출처의 일시적인 네트워크·408·425·429·5xx 장애: 최대 3회 재시도 후 해당 출처를 `skipped`로 기록하고, 다른 출처가 정상이면 Warning과 함께 정상 출처만 반영한다.
- 고정된 공식 YouTube Atom 주소의 HTTP 404: 일시 장애로 보고 최대 3회 재시도한 뒤 YouTube만 건너뛴다.
- 두 출처 모두 일시 장애 또는 재시도 대상이 아닌 HTTP 오류: 전체 동기화를 실패시키고 파일을 반영하지 않는다.
...
fetch blog feed and classify availability
fetch youtube uploads and classify availability

if one source is temporarily unavailable:
    warn and continue with the available source
...
- 네이버 RSS와 YouTube Atom RSS를 모두 조회하지 못함
- 재시도 대상이 아닌 HTTP 오류
- YouTube Atom RSS XML 파싱 오류
- JSON 파싱 오류
- 예상하지 못한 데이터 형식
- 출처·호스트·승인 채널 불일치
- 정규화된 YouTube ID 충돌
...
한 출처의 일시 장애만 발생하면 정상 출처의 검증된 신규 항목은 하나의 JSON·이미지 교체 단위로 반영한다. 이는 손상된 파일의 부분 저장을 허용하는 것이 아니라, 서로 독립된 출처의 가용성을 격리하는 정책이다. 장애 출처가 복구되면 다음 피드 응답에 아직 포함된 누락 항목을 기존 ID와 비교해 추가한다. RSS는 전체 과거 backfill 원본이 아니므로 장기 장애 뒤 피드에 남지 않은 항목은 YouTube Data API 등 별도 보충 절차를 검토한다.
...
최대 3회
지수 backoff
HTTP 408/425/429/5xx 중심
고정 공식 YouTube Atom 주소의 404 포함
```
...
- YouTube Atom 404 → 3회 재시도 후 YouTube만 `skipped`, 네이버 정상 결과 반영
- 네이버 일시 장애 → YouTube 정상 결과 반영
- 다음 정상 실행 → 피드에 포함된 누락 항목 추가
- 두 출처 일시 장애 또는 비재시도 HTTP 오류 → 전체 실패·파일 무변경
...
- [ ] 한 출처 일시 장애 시 정상 출처만 반영하고 Actions summary에 `skipped`·Warning 기록
- [ ] YouTube Atom 404 3회 재시도와 다음 정상 실행의 누락분 추가 검증
- [ ] 두 출처 모두 일시 장애 또는 비재시도 HTTP 오류 시 전체 실패·파일 무변경
