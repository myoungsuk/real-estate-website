...
| 작성 기준일 | 2026-08-25 |
| 문서 상태 | 기준 사이트 핵심 공개 흐름·관리자 편집·GitHub 저장 API 구현 반영 |
...
| 외부 콘텐츠 | 승인된 메타데이터·원문 링크·자체 저장 썸네일 | 링크 미리보기는 보조 기능이며 운영자가 최종 확인 |
...
- 단지: slug, 동구 내 지역, 단지명, 표시 마크, 소개 제목·문단, 사진, 기본 사실 배열, 생활 특징 배열, 복수 HTTPS 출처, 기준일
- FAQ: `id`, 질문, 답변, 노출 순서, 공개 여부
- 외부 콘텐츠: 고유 ID, `draft|published`, 블로그·유튜브 유형, 제목, 게시일, 짧은 요약, 원문 URL, 자체 저장 썸네일과 대체 텍스트. 공개 목록은 최신순으로 정렬하고 블로그는 페이지당 9개, 유튜브는 페이지당 6개로 표시한다.
...
블로그와 유튜브는 각각 최신순으로 정렬해 블로그는 페이지당 9개, 유튜브는 페이지당 6개씩 표시한다. 페이지 번호는 카드 그리드 아래 중앙에 배치하며 키보드 버튼, 현재 페이지 `aria-current`, 이전·다음 비활성 상태를 제공한다.

관리자 화면은 블로그와 유튜브를 독립된 카테고리로 나누고 카테고리별 건수, 제목·URL 검색, 새 항목 등록과 수정 목록을 제공한다. 저장 형식과 API는 공통 `external-links` 리소스를 유지한다.
...
- 홈은 50건 전체를 사진 없는 카드로 6건씩 페이지 처리하고, 홈에서 기본순·가격순·최신순·면적순 정렬을 제공한다. 기본순을 누르면 원래 네이버 데이터 순서와 1페이지로 복원한다. 매물 목록은 거래유형·매물유형 필터와 가격순·최신순·면적순 정렬을 제공하며, 카드는 같은 매물번호의 `fin.land.naver.com/articles/` 상세 주소를 새 탭으로 연다.
...
쓰기 요청은 같은 Origin, `application/json`, 2시간 이내 HMAC CSRF 토큰을 요구한다. GitHub 파일은 최신 SHA가 일치할 때만 수정하고 충돌 시 `409`로 중단한다. 허용 파일은 `listings.json`, `office.json`, `complexes.json`, `external-links.json`, `home-content.json`, `faq.json`, `reviews.json`으로 고정한다. `naver-listings.json`은 현재 운영자 확인 스냅샷으로 관리하며 관리자 쓰기 허용 목록에는 추가하지 않는다.
