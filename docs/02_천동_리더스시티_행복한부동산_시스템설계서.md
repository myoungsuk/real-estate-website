...
- 홈은 전체 매물을 사진 없는 카드로 6건씩 페이지 처리하고, 홈에서 최근 등록순·가격 낮은순·가격 높은순·면적 작은순·면적 큰순 정렬을 제공한다. 최근 등록순을 기본값으로 사용하며 필터 초기화도 이 상태로 돌아간다. 면적 정렬은 카드의 첫 번째 표시면적을 사용하므로 서로 다른 유형을 비교할 때는 매물유형 필터를 먼저 적용한다. 매물 목록도 같은 정렬 기준과 거래유형·매물유형 필터를 제공하며, 카드는 `등록 YYYY.MM.DD`와 같은 매물번호의 `fin.land.naver.com/articles/` 상세 주소를 표시한다.
...
쓰기 요청은 같은 Origin, `application/json`, 2시간 이내 HMAC CSRF 토큰을 요구한다. JSON 저장은 GitHub branch tip을 한 번 조회하고 그 commit/tree에 속한 허용 JSON 전체를 읽어 교차 검증한다. 단일 저장은 기존 `/api/admin/v1/content/:resource`, 복수 저장은 `PUT /api/admin/v1/content`를 사용한다. 요청된 모든 대상 blob SHA가 화면의 SHA와 일치해야 하며, 복수 후보를 전체 허용 JSON 상태에 함께 적용해 검증한 뒤 base tree에 하나 이상의 대상 blob을 넣은 단일 commit을 만들고 `force: false` ref 갱신으로 branch 이동을 CAS한다. SHA 불일치나 snapshot 이후 다른 commit의 선반영은 Git 쓰기 전 또는 ref 갱신에서 `409`로 중단한다. ref PATCH가 `422`이면 현재 tip을 다시 읽어 기준 commit에서 실제로 이동한 경우만 `409`로 바꾸고, tip이 같거나 재조회에 실패하면 원래 GitHub 장애 `502`를 유지한다. 허용 파일은 `listings.json`, `naver-listings.json`, `office.json`, `complexes-overview.json`, `complexes.json`, `external-links.json`, `home-content.json`, `faq.json`, `reviews.json`으로 고정한다. `naver-listings.json` 저장도 숫자 ID·고정 source·ID 일치 네이버 URL·공개 텍스트 개인정보 검증을 통과해야 한다.
...
- 변경 가능 경로는 `.github/bank-listing-sync-state.json`과 `src/data/naver-listings.json` 두 개뿐이다. 외부 조회·테스트·빌드는 `contents: read` 검증 job에서 수행하고, 별도 배포 job만 검증 artifact를 `contents: write`로 push한다.
