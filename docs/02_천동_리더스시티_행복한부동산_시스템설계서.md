...
| 작성 기준일 | 2026-09-01 |
| 문서 상태 | 신흥 SK뷰 확장 데이터 계약·정규화 매칭·관리자 파서·공개 상태와 로컬·CI·Production 검증 및 IndexNow 알림 완료, 실기기 검수 제외 승인 |
...
> 신흥 SK뷰 공개 문구와 운영자 직접 촬영 대표 사진은 2026-09-01 승인본을 적용한다. 다른 미확정 공개 정보와 법정 표시사항은 운영자 승인 후 적용하며, 무료 플랜의 약관과 한도는 출시 시점에 다시 확인한다.
...
4. GitHub 변경 이력, 로컬 화면과 pull request CI로 공개 전 검수한다.
...
| 호스팅 | Cloudflare Workers Static Assets Free | 정적 자산 호스팅·SSL·CDN·Git 연동 Production 배포 |
...
flowchart TD
    ...
    GIT -->|pull request| CI[GitHub Actions 검증]
    GIT -->|master push| CF[Cloudflare Workers Builds]
...
- 홈 콘텐츠: 대표 소개, 원본 비율을 보존하는 리더스시티 현장 사진 영역, 리더스시티·신흥 SK뷰 상담 범위 제목·설명 카드와 자체 이미지 경로
- 단지 전체 안내: `complexes-overview.json` 객체에 Hero 문구, 홈·목록 카드 순서 `featuredComplexSlugs`, 비교표 열 순서 `comparisonComplexSlugs`, 리더스시티 숫자 카드와 4·5블록 비교행, 공통 생활권·현장 확인사항, 관련 콘텐츠 ID, 출처 메타데이터와 기준일 저장
- 단지: `complexes.json` 배열에 slug, 동구 내 지역, 단지명, 다른 실제 명칭 `aliases`, 데이터 기반 `seo.title`·`seo.description`, `unitDataNote`, 표시 마크, 소개 제목·문단, 사진, 기본 사실, 면적별 세대수, 공급 요약, 생활환경, 시설 확인 상태, 현장 확인사항, FAQ, 관련 콘텐츠 ID, 출처 메타데이터와 기준일 저장. 세 신규 필드는 `preparing|published` 모두 항상 명시
- 신흥 SK뷰는 `preparing`으로 먼저 저장했으며 대표 사진·1차 출처·면적 및 공급 합계·시설 상태 라벨·FAQ·확인일·운영자 문구 승인 Gate를 통과한 2026-09-01 변경에서 `published`로 전환
- 신흥 SK뷰 준비 데이터에는 공식 확인한 신흥동 161-33, 12개동·1,588세대, 사용승인일 2022-04-28, 분양 1,499세대·임대 89세대와 K-apt·대전 동구청·대전광역시 원문을 저장한다. 공개 전환 시 면적 합계 1,588, 공급 합계와 분양·임대 수량, 공식 사실·출처 ID를 validator가 재검증한다.
- 공개 단지 SEO는 제목 70자·설명 180자를 상한으로 하고 준비 안내 문구가 남은 공개 데이터를 거부한다. 대표 사진은 `/images/area/<slug>.webp`와 `-640.webp`·`-1200.webp` 파생본이 모두 있어야 하며, 관리자 단일 WebP 업로드는 초안 검토 경로로만 사용한다.
...
- FAQ: 허용 카테고리, 질문, 답변을 배열 순서대로 저장하며 공개 화면은 8개 카테고리 바로가기와 구역별 아코디언으로 표시
...
- 단지 필터와 상세의 현재 매물 연결은 `published` 단지의 표시명과 alias를 NFKC·영문 소문자·공백·점·가운데점·하이픈 제거 방식으로 정규화하고 가장 긴 접두어를 우선한다. 같은 길이 복수 일치는 연결하지 않으며 매칭 실패 매물은 일반 매물로 유지한다.
- 거래유형·매물유형·단지·가격·면적·정렬 query는 허용값과 상한을 정규화하며, 월세 보증금과 월 임대료는 독립 조건으로 처리한다.
...
| `InquiryComposer` | 선택 공개 매물과 희망 조건의 서버 비저장 문의 문장 생성 |
...
- GitHub Actions 매시간 예약 실행과 수동 실행에서는 공식 블로그 ID `p5468300`의 HTTPS RSS에 노출된 신규 글만 감지한다. RSS에 없는 글을 감지한다고 표현하지 않고 기존 항목을 삭제하지 않는다.
...
- `naver-listings.json` 루트에는 매물 목록 업데이트일 `checkedAt`을 두고, 각 항목에는 개별 매물번호·유형·거래유형·가격·면적·동 번호·광고 층수·방향·설명·부동산뱅크 등록기간 시작일 `registeredAt`·상세 URL을 저장한다.
- 홈은 전체 매물을 사진 없는 카드로 6건씩 페이지 처리하고, 홈에서 최근 등록순·가격 낮은순·가격 높은순·면적 작은순·면적 큰순 정렬을 제공한다. 최근 등록순을 기본값으로 사용하며 필터 초기화도 이 상태로 돌아간다. 면적 정렬은 카드의 첫 번째 표시면적을 사용하므로 서로 다른 유형을 비교할 때는 매물유형 필터를 먼저 적용한다. 매물 목록은 같은 정렬 기준과 거래유형·매물유형·단지·가격·면적 조건, 관심 매물 최대 30개, 비교 최대 3개와 비저장 문의 문장을 제공하며, 카드는 `등록 YYYY.MM.DD`와 같은 매물번호의 `fin.land.naver.com/articles/` 상세 주소를 표시한다.
...
- 빌드 검증은 중복 매물번호, HTTPS 호스트, URL 경로의 매물번호 일치, 필수 표시값, 등록일과 목록 업데이트일의 날짜 관계를 검사한다.
...
- 홈: 사이트명용 `WebSite`와 승인 정보 기반 `RealEstateAgent`·`LocalBusiness`를 고정 `@id`로 연결한 단일 `@graph`
- 중개사무소 `sameAs`: 승인된 네이버 플레이스·공식 네이버 블로그·공식 YouTube 채널만 사용
- 홈을 제외한 공개 HTML 탐색 경로: 화면 계층과 같은 절대 URL의 `BreadcrumbList`
...
- 현재 Production 이외 브랜치 빌드는 비활성화되어 있다. 나중에 Preview를 활성화하면 검색 색인을 차단하고 Production Secret을 연결하지 않는다.
- 404·중복 canonical·내부 링크를 빌드 후 검사한다.

### 12.4 검색 변경 알림

- 네이버 IndexNow 공개키는 `public/<key>.txt`에 두며 인증 비밀값으로 취급하지 않는다.
- 성공한 `master` push CI의 변경 파일을 `scripts/indexnow.mjs`가 현재 Production sitemap URL과 연결한다.
- 공통 레이아웃·컴포넌트 변경은 sitemap 전체, 데이터 변경은 관련 목록·상세 URL만 제출 대상으로 계획한다.
- `.github/workflows/notify-indexnow.yml`은 Cloudflare의 `search` deployment marker와 공개키가 확인된 뒤에만 네이버 IndexNow에 POST한다.
- IndexNow는 sitemap·Search Console·서치어드바이저 수집 요청을 대체하지 않으며 수집·색인을 보장하지 않는다.
...
- 브라우저 로컬 저장소에는 관심 기능에 필요한 공개 매물 ID만 schemaVersion과 함께 최대 30개 저장한다. 비교 ID는 URL query에 최대 3개만 사용하고, 상담 조건·자유 메모·생성 문장은 로컬 저장소·URL·서버에 저장하지 않는다.
...
공개 정적 경로는 `ASSETS`가 직접 제공하고 `/api/admin` 계열만 Worker를 먼저 실행한다. 관리자 API는 Cloudflare Access JWT를 서버에서 다시 검증한다. 기본 상태에서는 `health`, `session`, `system`, `deployment-status`를 제공하며, Production Secret이 모두 준비된 경우 허용 콘텐츠 조회·사전 검증·저장·변경 이력·안전 복원, WebP 이미지 업로드와 네이버 블로그·유튜브 링크 미리보기를 추가로 제공한다. `deployment-status`는 같은 Worker의 ASSETS binding에서 marker v2를 `no-store`로 읽으므로 Cloudflare API 토큰을 추가하지 않는다.

쓰기와 사전 검증 요청은 같은 Origin, `application/json`, 2시간 이내 HMAC CSRF 토큰을 요구한다. `POST /api/admin/v1/content/validate`는 GitHub branch tip을 한 번 조회하고 그 commit/tree에 속한 허용 JSON 전체를 읽어 후보를 교차 검증하되 blob·tree·commit·ref를 만들지 않는다. 실제 단일 저장은 기존 `/api/admin/v1/content/:resource`, 복수 저장은 `PUT /api/admin/v1/content`를 사용한다. 요청된 모든 대상 blob SHA가 화면의 SHA와 일치해야 하며, 복수 후보를 전체 허용 JSON 상태에 함께 적용해 검증한 뒤 base tree에 하나 이상의 대상 blob을 넣은 단일 commit을 만들고 `force: false` ref 갱신으로 branch 이동을 CAS한다. SHA 불일치나 snapshot 이후 다른 commit의 선반영은 Git 쓰기 전 또는 ref 갱신에서 `409`로 중단한다. ref PATCH가 `422`이면 현재 tip을 다시 읽어 기준 commit에서 실제로 이동한 경우만 `409`로 바꾸고, tip이 같거나 재조회에 실패하면 원래 GitHub 장애 `502`를 유지한다. 허용 파일은 `listings.json`, `naver-listings.json`, `office.json`, `complexes-overview.json`, `complexes.json`, `external-links.json`, `home-content.json`, `faq.json`, `reviews.json`으로 고정한다. 단지 관리자는 alias·SEO·단위 안내와 featured·comparison 순서를 편집하며 비교행 열 수를 선택된 비교 대상 수와 정확히 맞춘 뒤 같은 전체 후보 검증을 통과해야 한다. `naver-listings.json` 저장도 숫자 ID·고정 source·ID 일치 네이버 URL·공개 텍스트 개인정보 검증을 통과해야 한다.

`GET /api/admin/v1/content-history/:resource`는 허용 경로와 현재 `master`에 포함된 commit만 최대 10개씩 cursor로 조회하며 이메일·연락처·긴 민감 문자열을 가린 제목·작성자 표시 이름·시각·commit/blob SHA만 반환한다. 과거 버전 상세 조회는 source commit이 현재 branch의 조상인지 확인한 뒤 해당 허용 blob만 읽고 현재 전체 JSON과 교차 검증한다. `POST /api/admin/v1/content/restore`는 정확한 resource 확인 문구, 현재 blob SHA, 현재 branch-tip을 다시 확인한 뒤 과거 JSON을 base tree에 넣은 새 commit을 `force: false`로 만든다. schema 불일치·변경 없음·SHA 또는 ref 이동은 쓰지 않고 중단하며, 복원 성공 commit과 resource digest는 일반 배포 상태 화면에서 Production 일치까지 추적한다. Cloudflare 전체 버전 롤백은 이 콘텐츠 복원 API와 연결하지 않는다.

### 15.2 CI 검사

- `npm ci`로 의존성과 잠금 파일 일치 확인
- `npm test`와 `npm run check`로 단위 테스트, TypeScript·Astro, 콘텐츠 스키마·중복·허용 경로 검사
- 기본 `noindex` 정적 빌드
- `PUBLIC_SITE_URL=https://leaderscityhappy.com`, `PUBLIC_ALLOW_INDEXING=true`를 사용한 별도 Production-mode 빌드
- Production 산출물의 robots, sitemap, canonical, JSON-LD·BreadcrumbList, IndexNow 공개키, 관리자 `noindex`, `deployment-marker.json` 검사
- marker v2의 Workers Builds·GitHub Actions source commit/branch, 허용 관리자 JSON별 정규화 SHA-256 digest와 기존 네 scope hash 검사
- Production 산출물을 로컬 정적 서버로 제공해 Playwright Chromium으로 홈 CTA·매물 필터·404·360px 메뉴·가로 넘침 검사
- 홈·매물·단지 목록·신흥 SK뷰 상세·신흥 SK뷰 필터 결과의 모바일 Lighthouse 성능·접근성·SEO 90점, LCP 2.5초·TBT 200ms·CLS 0.1 기준 검사
- 공식 GitHub Action을 검증한 commit SHA로 고정하고 checkout 자격 증명 저장 차단
- 성공한 `master` CI 완료 이벤트만 읽기 권한으로 받아 Production `search` marker 확인 뒤 IndexNow 알림

핵심 Playwright E2E와 Lighthouse는 CI에 포함한다. 200% 확대 동등 조건과 키보드는 로컬 브라우저로 검증한다. 이번 신흥 SK뷰 공개의 실제 Android Chrome·iPhone Safari 검수는 2026-09-01 운영자 승인으로 완료 조건에서 제외하며, 전화·문자·카카오 앱 전환과 현장 네트워크의 INP는 필요 시 운영 후 별도 확인한다.

### 15.3 외부 콘텐츠 예약·수동 동기화

- `.github/workflows/sync-external-content.yml`은 매시간 17분과 `workflow_dispatch`로 실행한다.
- 네트워크 수집·의존성·테스트·빌드는 `contents: read` 검증 job에서 수행한다. 변경된 허용 파일 artifact를 적용하고 push하는 별도 job만 `contents: write`를 사용하며, 토큰은 최종 push 단계에만 전달한다.
- 콘텐츠 변경 경로는 `src/data/external-links.json`, `public/images/blog/`, `public/images/youtube/`로 제한한다. 콘텐츠가 없고 마지막 활동 후 45일 이상이면 `.github/automation-health.json`만 별도 keepalive 커밋으로 갱신한다.
- 콘텐츠와 keepalive는 섞어 커밋하지 않는다. 허용 경로 밖의 변경이나 검증 뒤 `master` 변경이 있으면 push 전에 실패한다.
...
- push 뒤 Production의 `external` 또는 `automation` marker가 예상 SHA-256 값과 일치할 때까지 확인하며 시간 초과·불일치를 실패로 표시한다.
...
- 변경 가능 경로는 `.github/bank-listing-sync-state.json`, `.github/listing-review-state.json`, `src/data/naver-listings.json` 세 개뿐이다. 외부 조회·테스트·빌드는 `contents: read` 검증 job에서 수행하고, 별도 배포 job만 검증 artifact를 `contents: write`로 한 커밋에 push한다.
- 첫 실행은 현재 부동산뱅크 ID를 기준선으로 만들고, 이후 기준선에서 빠진 ID만 삭제한다. 다른 공급처 네이버 ID는 유지한다.
- 전체 공개 목록 수집이 정상 완료된 경우에만 Bank 매물의 `lastSeenAt`을 갱신한다. `.github/listing-review-policy.json`의 출처별 경고 일수가 `null`이면 관리자 경고를 비활성화하며 자동 종료·삭제·공개 숨김은 실행하지 않는다.
- 직접 등록 매물의 단건 `확인 완료`는 `.github/listing-review-state.json`의 `lastReviewedAt`만 공통 변경 미리보기·서버 전체 검증·최신 blob SHA·non-force Git commit으로 갱신한다. 잘못된 확인은 관리자 변경 이력에서 과거 상태를 새 복원 커밋으로 되돌린다.
- 첫 Production 실행은 2026-08-27 자동 커밋 `2c885955`에서 50건 기준선과 공개 반영을 확인했다.
- push 뒤 Production의 `bank` marker가 예상 SHA-256 값과 일치할 때까지 확인하며 시간 초과·불일치를 실패로 표시한다.

### 15.5 배포 흐름

1. 작업 브랜치 또는 로컬 수정
2. 로컬 화면과 pull request CI의 기본·Production-mode 빌드 검사
3. 운영자 모바일·PC 검수
4. 기본 브랜치 반영
5. Production 자동 배포
6. 핵심 화면·CTA·404·사이트맵·배포 marker 점검
7. 관리자 저장인 경우 commit SHA와 Production resource digest 일치 확인

### 15.6 롤백

- 먼저 Cloudflare의 직전 정상 배포 버전으로 되돌려 공개 화면을 복구한다. 이어서 문제 커밋을 `git revert`하고 재배포해 `master`·Production Git SHA·marker를 다시 일치시킨다. 강제 push로 `master` 이력을 다시 쓰지 않는다.
...
- 미분양 물량 문맥의 천 단위 쉼표 수량(예: `미분양 물량 2026년 7월 1,866호`)은 허용하되 단독·동 결합 호수, 호실, 같은 문자열의 다른 민감정보는 차단한다. RSS 병합 결과도 저장 전에 공통 스키마·금지 필드·민감 문자열 검사를 수행하고, 승인 공개값은 같은 `office.json`에서 읽는다.
...
4. `draft`로 로컬 화면에서 확인하고 pull request CI를 통과시킨다.
...
3. 마지막 정상 배포와 현재 Production의 Git SHA·`deployment-marker.json` 비교
...
| ADR-004 | DB 없이 Workers Static Assets와 관리 API를 같은 Worker로 운영 | 승인, 편집 UI·GitHub 저장 API 구현·Production 운영 쓰기 활성화 |
...
- [x] 키보드·200% 확대 동등 조건·명도 대비·대체 텍스트 확인
- [x] Android Chrome·iPhone Safari 실기기 검수는 이번 신흥 SK뷰 공개 완료 조건에서 제외 — 운영자 승인, 2026-09-01
...
- [x] 사이트맵·robots.txt·404와 `/sitemap.xml`의 정확한 301 별칭 확인
- [x] 구조화 데이터에 실제 승인 정보만 포함하고 WebSite·중개사무소 엔터티 연결
- [ ] 모바일 Lighthouse 성능·접근성·SEO 90 이상 목표 확인

### 배포·운영

- [x] 로컬 검증 → GitHub `master` CI → Cloudflare Production 자동 배포 확인 — 신흥 SK뷰 릴리스, 2026-09-01
