...
쓰기 요청은 같은 Origin, `application/json`, 2시간 이내 HMAC CSRF 토큰을 요구한다. JSON 저장은 GitHub branch tip을 한 번 조회하고 그 commit/tree에 속한 허용 JSON 전체를 읽어 교차 검증한다. 대상 blob SHA가 화면의 SHA와 다르면 `409`로 중단하며, 일치하면 base tree에 대상 blob 하나를 교체한 단일 commit을 만든 뒤 `force: false` ref 갱신으로 branch 이동을 CAS한다. snapshot 이후 다른 commit이 먼저 반영되면 ref 갱신을 `409`로 중단한다. 허용 파일은 `listings.json`, `naver-listings.json`, `office.json`, `complexes-overview.json`, `complexes.json`, `external-links.json`, `home-content.json`, `faq.json`, `reviews.json`으로 고정한다. `naver-listings.json` 저장도 숫자 ID·고정 source·ID 일치 네이버 URL·공개 텍스트 개인정보 검증을 통과해야 한다.
