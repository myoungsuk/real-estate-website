...
- 외부 콘텐츠: 고유 ID, `draft|published`, 블로그·유튜브 유형, YouTube의 `youtubeFormat: video|short`, 제목, 게시일, 짧은 요약, 원문 URL, 자체 저장 썸네일과 대체 텍스트. 공개 목록은 최신순으로 정렬하고 블로그는 페이지당 9개로 표시한다. 유튜브는 전체·일반 영상·Shorts 필터를 한 목록에서 제공하고 홈은 선택 형식의 최신 6개만, `/youtube/`는 선택 형식을 페이지당 6개씩 표시한다.
...
블로그와 유튜브는 각각 최신순으로 정렬한다. 블로그는 페이지당 9개를 표시하고, 유튜브는 전체·일반 영상·Shorts 필터를 한 목록에서 전환한다. 홈은 선택 형식의 최신 6개만 표시하며 `/youtube/`는 선택 형식을 페이지당 6개씩 표시한다. 필터 전환 시 1페이지로 초기화한다. 페이지 번호는 카드 그리드 아래 중앙에 배치하며 키보드 버튼, 현재 페이지 `aria-current`, 이전·다음 비활성 상태를 제공한다.
...
- `naver-listings.json`에 확인일과 개별 매물번호·유형·거래유형·가격·면적·동 번호·광고 층수·방향·설명·확인일·상세 URL을 저장한다.
- 홈은 50건 전체를 사진 없는 카드로 6건씩 페이지 처리하고, 홈에서 기본순·가격 낮은순·가격 높은순·최근 확인순·면적 작은순·면적 큰순 정렬을 제공한다. 기본순을 누르면 원래 네이버 데이터 순서와 1페이지로 복원한다. 최근 확인순은 네이버 등록일이 아니라 `confirmedAt` 확인일을 사용한다. 면적 정렬은 카드의 첫 번째 표시면적을 사용하므로 서로 다른 유형을 비교할 때는 매물유형 필터를 먼저 적용한다. 매물 목록도 같은 정렬 기준과 거래유형·매물유형 필터를 제공하며, 카드는 같은 매물번호의 `fin.land.naver.com/articles/` 상세 주소를 새 탭으로 연다.
...
- `/admin/listings/`와 `/admin/listings/editor/`는 자체 매물 CRUD 대신 네이버 공개 목록 확인·검색·유형 필터와 `office.naverListingsUrl` 외부 관리 동선을 제공한다.
...
쓰기 요청은 같은 Origin, `application/json`, 2시간 이내 HMAC CSRF 토큰을 요구한다. GitHub 파일은 최신 SHA가 일치할 때만 수정하고 충돌 시 `409`로 중단한다. 허용 파일은 `listings.json`, `office.json`, `complexes-overview.json`, `complexes.json`, `external-links.json`, `home-content.json`, `faq.json`, `reviews.json`으로 고정한다. `naver-listings.json`은 현재 운영자 확인 스냅샷으로 관리하며 관리자 쓰기 허용 목록에는 추가하지 않는다.
