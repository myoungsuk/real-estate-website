...
원본 엑셀은 서버나 GitHub로 전송되지 않습니다. 자동 동기화도 공개 목록의 카드 정보와 매물번호만 사용하며 소유자·매도자·전화번호·정확한 호수·내부 메모를 결과에 넣지 않습니다. 직전 부동산뱅크 기준선에서 빠진 매물은 자동 삭제하지만, 기준선에 없던 다른 공급처 매물은 유지합니다. 한방광고 엑셀, 네이버 크롤링, 로그인·상세페이지·사진 수집은 지원하지 않습니다.
...
- 공개 카드는 게시일 최신순으로 정렬됩니다. 블로그는 9개 단위로 페이지가 나뉩니다. 홈의 유튜브는 일반 영상 최신 6개만 표시합니다. `/youtube/`는 전체 혼합 필터 없이 일반 영상을 기본으로 `일반 영상·Shorts 보기`를 전환하며 선택 형식을 6개 단위로 나눕니다. 필터를 바꾸면 1페이지로 돌아갑니다.
...
- 자동 동기화 전 기준 데이터는 공식 네이버 블로그의 2024~2026년 공개 게시글 128건(2024년 41건, 2025년 34건, 2026년 53건)과 `youtubeFormat: video`로 분류한 공식 유튜브 일반 영상 40건입니다. 2026-08-26에는 공식 채널 Shorts 탭의 공개 Shorts 32건을 전체 확인해 등록했습니다. 새 글·영상은 관리자 화면에서 직접 추가하거나 아래 자동·수동 동기화 워크플로로 추가할 수 있습니다.
...
RSS 결과는 JSON·썸네일 저장 전에 공개 스키마, 금지 필드와 민감 문자열을 함께 검사합니다. 제목·요약·썸네일 설명의 `미분양 물량 2026년 7월 1,866호`처럼 `미분양 물량` 바로 뒤에 연도·월(생략 가능)과 천 단위 쉼표 수량이 있는 경우만 통계로 구분합니다. 단독 `1203호`·`1,203호`, `101동 1203호`, `호실`과 미승인 연락처는 계속 차단합니다. 공개 연락처·사무소 호수 예외는 `src/data/office.json`의 승인된 값만 사용합니다. 검증 실패 시 오류에 표시된 필드를 확인하며 검사를 끄거나 테스트를 생략하지 않습니다.
...
- `src/data/complexes-overview.json`: 홈·목록에 표시할 `featuredComplexSlugs` 순서와 비교표 열의 `comparisonComplexSlugs` 범위, 리더스시티 4·5블록 숫자 카드·비교표, 공통 생활권·현장 확인사항, 관련 공개 콘텐츠 ID, 출처와 확인일
- `src/data/complexes.json`: 대전 동구의 지역·단지별 표시명·다른 실제 명칭(`aliases`)·검색 제목과 설명(`seo`)·면적 자료 안내(`unitDataNote`), 사진, 기본 사실, 면적별 세대 구성, 공급 요약, 생활환경, 커뮤니티 확인 상태, 현장 확인사항, FAQ, 관련 공개 콘텐츠 ID, 복수 출처와 확인일
- 단지를 `published`로 공개하려면 사진 대체 텍스트, 기본 사실, 면적·공급·생활환경·시설 확인 상태, FAQ, 하나 이상의 HTTPS 출처와 확인일이 필요합니다. SEO 제목은 최대 70자, 설명은 최대 180자이며 준비 상태 안내 문구를 최종 SEO·소개 문구에 남길 수 없습니다.
- `aliases`에는 공백 차이가 아니라 실제 다른 명칭만 한 줄에 하나씩 입력합니다. 매물 제목은 NFKC·영문 대소문자·공백·점·가운데점·하이픈을 정규화한 뒤 가장 긴 단지명 접두어로 연결합니다.
- `featuredComplexSlugs`에 `preparing` 단지를 미리 넣을 수 있지만 공개 홈·단지 목록·매물 필터에는 나타나지 않습니다. `comparisonComplexSlugs`에는 공개 단지만 두며 비교표 각 행은 선택한 열 수와 순서를 정확히 맞춥니다.
- 신흥 SK뷰는 운영자가 직접 촬영하고 공개를 승인한 대표 사진, 공식 면적·공급 합계, 시설 상태 라벨, 네 공식 출처·확인일과 승인 문구를 반영해 2026-09-01 `published`로 전환했습니다. 상세 HTML·sitemap·매물 단지 필터에 포함되며 실제 시설 운영 여부는 현장 재확인 대상으로 표시합니다.
- 신흥 SK뷰 공개 데이터에는 지번 `신흥동 161-33`, 도로명주소 `대전광역시 동구 충무로 255`, 12개동·1,588세대, 사용승인일 2022-04-28, 60㎡ 이하 897세대·60㎡ 초과~85㎡ 이하 691세대, 분양 1,499세대·임대 89세대, 개별난방, 지하주차 1,957대와 공식 등록 시설을 기록했습니다. 최고·최저 층수와 시설의 현재 실제 운영 여부는 확정하지 않습니다.
- 신흥 SK뷰 공개 상태에서는 면적별 세대수 합계 1,588, 공급 합계 1,588 및 분양 1,499·임대 89, 위 공식 사실과 네 출처가 모두 일치해야 합니다. 검증기는 하나라도 다르면 저장·빌드를 중단합니다.
- 공개 단지 대표 사진은 `public/images/area/<slug>.webp` 원본과 `-640.webp`, `-1200.webp` 파생본을 함께 준비합니다. 관리자 화면에서 올린 `public/images/content/area/` 단일 WebP는 초안 검토용이며, 파생본을 만들고 권리·화면을 확인하기 전에는 공개 대표 사진으로 사용할 수 없습니다.
...
- `src/data/naver-listings.json`에는 목록 업데이트일 `checkedAt`과 네이버페이 부동산에 공개된 개별 매물번호, 매물유형, 거래유형, 가격, 면적, 동 번호, 광고 층수, 방향, 설명, 부동산뱅크 등록기간 시작일 `registeredAt`, 상세 URL을 기록합니다.
- 홈은 전체 매물을 사진 없는 카드로 6건씩 페이지 처리하고 `/properties/`는 전체 카드를 표시합니다. 홈과 전체 목록은 최근 등록순, 가격 낮은·높은순, 면적 작은·큰순 정렬을 제공하며 최근 등록순이 기본값입니다. 면적은 카드의 첫 번째 표시면적 기준이므로 서로 다른 유형을 비교할 때는 매물유형 필터를 먼저 선택합니다. 목록 상단에는 `매물 목록 업데이트: YYYY.MM.DD`, 카드에는 `등록 YYYY.MM.DD`를 표시하고 각 카드는 `https://fin.land.naver.com/articles/<매물번호>`로 바로 이동합니다.
- 부동산뱅크 자동 동기화 또는 관리자 수동 등록·수정·등록 종료로 `checkedAt`과 공개 항목이 바뀌면 GitHub 커밋과 Cloudflare 자동 배포 후 공개 화면에 표시됩니다. 자동 동기화 상세 절차는 `docs/operations/BANK_LISTING_SYNC.md`를 따릅니다.
...
- `src/data/faq.json`은 `category`, `question`, `answer` 필드를 사용하며, 공개 화면은 8개 카테고리 바로가기와 구역별 아코디언으로 38문항을 모두 표시합니다.
- 카테고리는 기존 8개 허용값 중 하나를 사용해야 하며, 빈 질문·답변과 중복 질문은 `npm run check`에서 거부됩니다.
