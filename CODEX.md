...
- 호수 검사는 `미분양 물량 [YYYY년 M월] 1,866호`처럼 명시적인 미분양 물량 문맥의 천 단위 쉼표 수량만 통계로 구분한다. 단독 호수·동 번호와 결합한 호수·호실·다른 민감정보는 계속 차단하며, RSS 동기화도 파일 저장 전에 공통 공개 스키마·금지 필드·민감 문자열 검사를 적용한다. 공개 연락처 예외는 같은 저장소의 `office.json`을 기준으로 한다.
