...
- 홈 콘텐츠: 대표 소개, 사무소 사진 영역, 리더스시티 알아보기 제목·설명 카드와 자체 이미지 경로
- 단지: slug, 동구 내 지역, 단지명, 표시 마크, 소개 제목·문단, 출처, 기준일, 이미지
- FAQ: `id`, 질문, 답변, 노출 순서, 공개 여부
- 외부 콘텐츠: 고유 ID, `draft|published`, 블로그·유튜브 유형, 제목, 게시일, 짧은 요약, 원문 URL, 자체 저장 썸네일과 대체 텍스트
...
쓰기 요청은 같은 Origin, `application/json`, 2시간 이내 HMAC CSRF 토큰을 요구한다. GitHub 파일은 최신 SHA가 일치할 때만 수정하고 충돌 시 `409`로 중단한다. 허용 파일은 `listings.json`, `office.json`, `complexes.json`, `external-links.json`, `home-content.json`, `faq.json`, `reviews.json`으로 고정한다.
